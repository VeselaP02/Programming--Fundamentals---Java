...
public class TreasureHunt {
    public static void main(String[] args) {
        ...
        while (!command.equals("Yohoho!")) {
            ...
            switch (commandPairs[0]) {
                case "Loot":
                    for (int i = 1; i < commandPairs.length ; i++) {
                        boolean isContained = false;
                        for (int j = 0; j < treasureChest.length ; j++) {
                            if (commandPairs[i].equals(treasureChest[j])) {
                                isContained = true;
                                break;
                            }
                        }
                        if (!isContained) {
                            String newChest = commandPairs[i] + " " + String.join(" ",treasureChest);
                            treasureChest = newChest.split(" ");
                        }
                    }
                    break;
                case "Drop":
                    int position = Integer.parseInt(commandPairs[1]);
                    ...
                    break;
                case "Steal":
                    int numberOfStealingItems = Integer.parseInt(commandPairs[1]);
